(function() {
    ...
    var N = (function() {
        function C() {
            var Z = Q.getJson();
            var c = Z.dateArray;
            M("cm").style.height = Z.lines * 38 + 110 + "px";
            M("cm").innerHTML = "";
            for (var a = 0; a < c.length; a++) {
                ...
                X.style.top = Math.floor(a / 7) * 58 + 2 + "px";
                var b = R("DIV");
                b.className = "so";
              
                b.style.color = ((a % 7) > 4 || c[a].isRest) ? "#313131": "#313131";
...
//                Y.innerHTML = c[a].showInLunar;
    ...
    //地址传值修改(第一次进入页面传值为空，所有的跳转都在地址上带上年月日)
    ...
    if(year==null&month==null){
    	 date = new Date();
    	 	...
    	 	var str =year+"-"+month+"-01 12:00:00";
    	    str = str.replace(/-/g,"/");
    	    date = new Date(str);
    }
