$(function() {
	...
	if (operatH == 'edit') {
		...
		app.$getJSON(app.LNRATCHG_VIEW + "/" + athNo).done(
				function(data) {
					if (app.isOK(data)) {
						console.log(data);
						$("#athInd").val(data.athInd);
						try {
							$('#athInd').get(0).disabled = true;
						} catch (e) {
						}
						$("#lonol").val(data.loNo).attr("readonly", true);
						$(".oldRatMod").val(data.oldRatMod);
						try {
							$('#oldRatMod').get(0).disabled = true;
						} catch (e) {
						}
						$("#oldBaRt").val(data.oldBaRt);
						$("#oldBaRt1").val(
								app.formatPercent(data.oldBaRt * 100)).attr(
								"readonly", true);
						$("#oldBaRtTyp").val(data.oldBaRtTyp);
						try {
							$('#oldBaRtTyp').get(0).disabled = true;
						} catch (e) {
						}
						$("#oldRatTyp").val(data.oldRatTyp);
						try {
							$('#oldRatTyp').get(0).disabled = true;
						} catch (e) {
						}
						$("#chgChl").val(data.chgChl);
						try {
							$('#chgChl').get(0).disabled = true;
						} catch (e) {
						}
						$("#oldRtAd").val(data.oldRtAd);
						$("#oldRtAd1").val(
								app.formatPercent(data.oldRtAd * 100)).attr(
								"readonly", true);
						...
						$("#oldExRt1").val(
								app.formatPercent(data.oldExRt * 100)).attr(
								"readonly", true);
						$("#oldOvRt").val(data.oldOvRt)
						$("#oldOvRt1").val(
								app.formatPercent(data.oldOvRt * 100)).attr(
								"readonly", true);
						$("#rtChgTyp").val(data.rtChgTyp)
								.attr("readonly", true);
						//利率调整间隔类型
						$("#rtChgPlTyp").val(data.rtChgPlTyp).attr("readonly",
								true);
						//利率调整间隔
						$("#rtChgPlNum").val(data.rtChgPlNum).attr("readonly",
								false);
						$("#newBaRtTyp").val(data.newBaRtTyp).attr("readonly", true);
						try {
							$('#newBaRtTyp').get(0).disabled = true;
						} catch (e) {
						}
						// 新执行利率
						...
						$("#newExRt1").val(
								app.formatPercent(data.newExRt * 100));
						// 新利率浮比
						...
						$("#newRtPe1").val(
								app.formatPercent(data.newRtPe * 100));
						// 新利率浮点
						...
						$("#newRtAd1").val(
								app.formatPercent(data.newRtAd * 100));

						$("#newBaRt").val(data.newBaRt);
						$("#newBaRt1").val(
								app.formatPercent(data.newBaRt * 100)).attr(
								"readonly", true);
						$("#newRatTyp").val(data.newRatTyp);
						try {
							$('#newRatTyp').get(0).disabled = true;
						} catch (e) {
						}
						$("#newRatMod").val(data.newRatMod).attr("readonly",
								true);
						$("#buzSeqNo").val(data.buzSeqNo)
								.attr("readonly", true);
						...
						$("#newOvPe1").val(
								app.formatPercent(data.newOvPe * 100));
						// 新罚息利率
		...
		app.$getJSON(app.LNRATCHG_VIEW + "/" + athNo).done(
				function(data) {
					if (app.isOK(data)) {
						...
						$("#rtChgPlNum").val(data.rtChgPlNum).attr("readonly",
								false);
						$("#newBaRtTyp").val(data.newBaRtTyp);
						try {
							$('#newBaRtTyp').get(0).disabled = true;
						} catch (e) {
						}
						...
						$("#newExRt").val(data.newExRt);
						$("#newExRt1").val(
								app.formatPercent(data.newExRt * 100)).attr(
								"readonly", true);
						// 新利率浮比
						if (data.newRtPe == null || data.newRtPe == "") {
							data.newRtPe = parseFloat("0");
						...
						$("#newRtAd").val(data.newRtAd);
						$("#newRtAd1").val(
								app.formatPercent(data.newRtAd * 100)).attr(
								"readonly", true);
						...
						$("#newRatMod").val(data.newRatMod).attr("disabled",
								true);
						$("#buzSeqNo").val(data.buzSeqNo)
								.attr("readonly", true);
	...
	$("select[name='newRatMod']").change(function() {
		rtChgTypFun($(this).val());
	});
	$("[name='rtChgTyp']").change(
			function() {
				var ss = $(this).val();
				if (ss == "GD") {
					$("[name=rtChgPlTyp]").addClass('required').attr(
							"disabled", false).parents('.form-group-hidden')
							.find('.control-label').addClass('fill');
					$("[name=rtChgPlNum]").addClass('required').attr("hidden",
							false).parent().siblings('label').addClass('fill');
					$("[name=nxtChgDt]").addClass('required').attr("disabled",
							false).parents('.selBox').siblings('label')
							.addClass('fill');
	...
	$('#dealBtn').on('click', function() {
		...
		if(editForm.valid()){
			...
			app.$post(app.LNRATCHG_MANUAL, param).done(function(data) {
				App.stopPageLoading();
				if (app.isOK(data)) {
					app.alertOK('提交成功!');
...
function rtChgTypFun(val){
	if(val=="FD"){
		...
		$('.form-rtChgTyp [name="rtChgTyp"]').removeClass('required');
	}
	$('select').selectpicker('refresh');
}
