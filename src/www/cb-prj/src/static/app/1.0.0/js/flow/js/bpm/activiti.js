+ function($, app){
  ...
  var eidtFlowInfo = function () {
    ...
    editModal.on('show.bs.modal', function (event) {
      ...
      var jqxhr = app.$get(app.base + '/bpm/flowinfo/' + flowId, 'json');
      if (!jqxhr) {
        return;
      }
  ...
  var removeFlowInfo = function() {
    ...
    $(".main-page").delegate('.remove', 'click', function(){
      ...
      bootbox.confirm({title: '提示', message: tmsg, callback: function (e) {
        if (e) {
          App.startPageLoading({ animate: true });
          var jqxhr = app.$post(app.base+'/bpm/flowremove/' + id, {}, 'json');
          if (!jqxhr) return false;
          jqxhr.done(function (data) {
            App.stopPageLoading();
            if (app.isOK(data)) {
    ...
    $('.activiti-create').on('click',function(){
      ...
      window.open(path+'/bpm/flowcreate','_blank');
    });
  ...
  /*流程查看*/
    ...
    var path = $(this).attr('data-path');
    window.open(path+'/workflow/flows/view/'+id,'_blank');
  });
  ...
  $(document).on('click','.activiti-edit',function(){
    var id = $(this).attr('data-activiti-id');
    var path = $(this).attr('data-path');
    window.open(path+'/workflow/flows/edit/'+id,'_blank');
  });
  ...
  $(document).on('click','.activiti-deploy',function(){
    ...
    $.ajax({
        url:base +"/workflow/flows/deploy/"+id,
        type:"POST",
        timeout:10000,
        async: true,
        dataType:"json",
        ...
        $('.ok', relviewModal).click(function () {
          ...
          for(var i=0; i<$('#relview-sel-table tr').length; i++) {
            ...
            if(relview.typeCode == '0') {
              $.each(fixrelview, function (j, obj) {
      ...
      var initPlugSelLoader = function(object) {
        $(".triggerStage", object).each(function(){
          $(this).empty().append('<option value="">请选择触发阶段...</option>').selectloader({
            triggerStage:  stepType == 'USERTASK' ?  taskTriggerStage:exectutriggerStage
          });
        });
