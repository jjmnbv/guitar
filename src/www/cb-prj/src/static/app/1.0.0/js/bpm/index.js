...
+ function ($, app) {
  ...
  $('#mainMenu').delegate('.nav-item', 'click', function () {
    ...
    if(action=='edit') {
      ...
      $('li .nav-item[data-action="save"]').attr('disabled','disabled');
      ...
      $('#flowChart').trigger('click');

      var flowNameVerify = function(obj, flowName) {
        var jqxhr = app.$get(app.FLOW_NAMEVERIFY + flowName);

        jqxhr.done(function (data) {
          if (app.isOK(data)) {
            if (!data.payload) {
              obj.val('').focus();
              app.alertOK('流程名称重复,请重新命名');
            }
            return;
          }
      ...
      $('#flowForm #pname input').on('change', function() {
        flowNameVerify($('#flowForm #pname input'), $('#flowForm #pname input').val());
      });
      ...
      if ($('div.flowlist > div.active').prop('id') == 'stopped') {
        app.alertError('流程已停用');
        return;
      }
      if ($('div.flowlist > div.active').prop('id') == 'notRelease') {
        app.alertError('流程尚未发布不可停用');
        return;
      }

      var selectId = getSelectId();
      if (selectId) {
        disableFlow(selectId);
      }
    }
            ...
            "columns": [
              ...
              {data: "statusStr"},
              {
                ...
                render: function (data) {
                  ...
                  return '<a href="#" class="flow-remove" data-flowinfo-id="' + data + '">删除</a>';
                }
              }
            ]
          });
          ...
          $('#flowChangeHistoryTable tbody').on('click', 'a.flow-remove', function () {
            ...
            $.ajax({
              ...
              success: function (data, textStatus, jqXHR) {
                flowChangeHistoryTable.ajax.reload();
              },
              error: function () {
                app.alertError('未知异常.');
              }
