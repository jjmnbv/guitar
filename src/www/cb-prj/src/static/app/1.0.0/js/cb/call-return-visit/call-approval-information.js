...
$(function () {
    ...
    +function ($, app) {
        var init = function () {
            ...
            $.Bpm.initBusFormExt(function (formId, busId) {
                if (formId == 'call-approval-information') {
                    applyWater = busId;
                }
            });
            App.stopPageLoading();
            ...
            $("#ul_div").html(html);
            $("#rollBack").hide();
            app.bindFormValidation($("#call-approval-information"));
            return;
        };
        init();
    ...
    $('#submitBtn').click(function () {
        ...
        if (taskStatus == "COMPLETED") {
            ...
            //判断必填项都已经填写
            $.getJSON(app.AJAX_CHECKINFOISRIGHT + applyWater + "/callback", function (data) {
                if (app.isOK(data)) {
                    if (!data.allRightFlag) {
                        app.alertError("有必填项未填写，请填写后再进行提交！");
                    } else {
                        var item = $("#call-approval-information");
                        if (!item.valid()) {
                        ...
                        if (opinionId == 'TH') {
                            $.Bpm.rollbackToWithTarget(opinionInfo, $("#rollBackNode").val(), function (data) {
                                if (app.isOK(data)) {
                                    app.alertOK('退回成功！');
                                    setTimeout(function () {
                                        window.location.href = "/cb/call-return-visit/return-visit-list.html";
                                    }, 2000);
                                } else {
