+ function($, app){
	...
	var postFormIds = function(formIdArr) {
		...
		$.ajax({
			...
			dataType : 'json',
			cache: false,
			url: app.base+'/bpm/formids/update',
	...
	var submitHandle = function() {
	    ...
	    if(!flag) {
			...
			ajaxPost(app.base+'/bpm/manualcommityn',
				'post',
				JSON.stringify({stepKey: stepKey, taskId: taskId}),
				false,
				function(data, textStatus, jqXHR) {
					if (data) {
						ajaxPost(app.base+'/bpm/nextstep/candidates',
							'post',
	...
	var rollback = function(){
		...
		$('#rollback_form .ok').on('click',function(){
			...
			$.ajax({
	  			...
	  			dataType : 'json',
	  			cache: false,
	  			url: app.base+'/bpm/rollback/'+taskId,
	...
	var activeTask = function(){
		...
		$(document).on('click','#activeTask',function(){
			$.ajax({
	  			...
	  			dataType : 'json',
	  			cache: false,
	  			url: app.base+'/bpm/active/'+taskId,
	  			success: function(data, textStatus, jqXHR) {
	  				if(app.isOK(data)) {
	  					App.stopPageLoading();
	  					app.alertOK('激活成功.');
