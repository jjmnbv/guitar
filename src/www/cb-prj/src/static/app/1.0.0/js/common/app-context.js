+function ($, app) {
    ...
    app.context = {
         ...
         */
         submit: function (option, app, url, text) {
            if (option.isEasyModal) {
                ...
                this._submitAjax(app, option.param, option.modal, option.url, option.text,option.contentType);
            }else {
         ...
         * @private
         */
        _submitAjax: function (app, param, modal, url, text,contentType) {
            app.$ajax(url, param, 'post',true,contentType,function (data) {
                if (app.isOK(data)) {
                    app.alertOK(text || '提交成功！');
                    modal.modal('hide');
                    app.loadData();
