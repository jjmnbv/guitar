+ function ($, app) {
  ...
  var copyFlowInfo = function () {
    ...
    $(".main-page").delegate('.fa-copy', 'click', function () {
      var selectedFlow = getSelectedFlow();
      if (!selectedFlow) {
        app.alertOK('未选择流程信息.');
        return;
      }
      //TODO copy flow
      App.startPageLoading({ animate: true });
      var jqxhr = app.$post(app.base + '' + selectedFlow.val(), {}, 'json');
      if (!jqxhr) return false;
      jqxhr.done(function (data) {
        App.stopPageLoading();
        if (app.isOK(data)) {
  ...
  var enableFlowHandle = function () {
    ...
    $(".main-page").delegate('.activiti-enable', 'click', function () {
      ...
      App.startPageLoading({ animate: true });
      var jqxhr = app.$post(app.base + '/bpm/flowenable/' + id, {}, 'json');
      if (!jqxhr) return false;
      jqxhr.done(function (data) {
        App.stopPageLoading();
        if (app.isOK(data)) {
  ...
  var disableFlowHandle = function () {
    $(".main-page").delegate('.activiti-disable', 'click', function () {
      ...
      App.startPageLoading({ animate: true });
      var jqxhr = app.$post(app.base + '/bpm/flowdisable/' + id, {}, 'json');
      if (!jqxhr) return false;
      jqxhr.done(function (data) {
        App.stopPageLoading();
        if (app.isOK(data)) {
  ...
  var reload = function () {
    $(".main-page").delegate('.Refresh-sign', 'click', function () {
      $('.main-page .pagination-reload').click();
    });
  };
  ...
  var removeFlowInfo = function () {
    ...
    $(".main-page").delegate('.delete-sign', 'click', function () {
      ...
      if (selectedFlow.data('runstatus') == '0') {
        app.alertOK('流程已启用无法删除.');
        return;
      }

      var tmsg = "确定删除选中的流程信息?";
      bootbox.confirm({
        title: '提示', message: tmsg, callback: function (e) {
          if (e) {
            App.startPageLoading({ animate: true });
            var jqxhr = app.$post(app.base + '/bpm/flowremove/' + selectedFlow.val(), {}, 'json');
            if (!jqxhr) return false;
            jqxhr.done(function (data) {
              App.stopPageLoading();
              if (app.isOK(data)) {
  ...
  /*新建流程*/
  $('.add-sign').on('click', function () {
    var path = $(this).attr('data-path');
    window.open(app.base + '/bpm/flowcreate', '_blank');
  });
  ...
  $(document).on('click', '.activiti-view', function () {
    ...
    var path = $(this).attr('data-path');
    window.open(app.base + '/workflow/flows/view/' + id, '_blank');
  });
  ...
  $(document).on('click', '.activiti-edit', function () {
    var id = $(this).attr('data-activiti-id');
    var path = $(this).attr('data-path');
    window.open(app.base + '/bpm/flowedit/' + id, '_blank');
  });
  ...
  $(document).on('click', '.activiti-deploy', function () {
    ...
    $.ajax({
      url: app.base + "/workflow/flows/deploy/" + id,
      type: "POST",
      timeout: 10000,
      async: true,
      dataType: "json",
      success: function (data, status, request) {
        if (top.app.isOK(data)) {
          $('.Refresh-sign', window.parent.document.body).click();
  ...
  /*流程复制*/
  $(document).on('click', '.copy-sign', function () {
    ...
    if (id) {
      $.ajax({
        url: app.base + "/bpm/flowcopy/" + id.val(),
        type: "POST",
        timeout: 10000,
        async: true,
        dataType: "json",
  ...
  var stepConfig = function () {
    ...
    stepListModal.on('show.bs.modal', function (event) {
      ...
      var jqxhr = app.$get(app.base + '/bpm/steps/' + flowInfoId, 'json');
      if (!jqxhr) {
        return;
      }
    ...
    var stepKey, stepName, stepDesc, stepType;
    ...
    stepConfigModal.on('show.bs.modal', function (event) {
      ...
      stepType = $(event.relatedTarget).data('stepType');
      if (!stepKey) return;

      $("strong", stepConfigModal).html(stepName + "-配置");
      App.startPageLoading({ animate: true });

      var jqxhr = app.$get(app.base + '/workflow/stepconfig/view?stepKey=' + stepKey + '&flowInfoId=' + flowInfoId, 'json');
      if (!jqxhr) {
        return;
      }
    ...
    $('.ok', stepConfigModal).click(function () {
      if (stepConfigForm.valid()) {
        App.startPageLoading({ animate: true });
        stepConfigForm.prop('action', app.base + '/workflow/stepconfig/update');
        if ($('input[name=id]', stepConfigForm).val() == null || $('input[name=id]', stepConfigForm).val() == '') {
          stepConfigForm.prop('action', app.base + '/workflow/stepconfig/create');
        }
    ...
    var relviewHandle = function () {
      ...
      var initRelviewtypeSelLoader = function (object) {
        ...
        $(".relviewtype", object).each(function () {
          $(this).empty().append('<option value="">请选择视图类型...</option>').selectloader({
            relviewtype: app.relviewtype
          });
          $(this).change(function () {
            if ($(this).val() == "1") {
              initRelviewSelLoader($(this).parent().parent().parent(), app.nonfixrelview);
            } else {
              initRelviewSelLoader($(this).parent().parent().parent(), app.fixrelview);
            }
          });
          $(this).trigger('change');
        });
      ...
      var initRelviewSelLoader = function (object, store) {
        $("select.relviewsel", object).each(function () {
      ...
      relviewModal.on('show.bs.modal', function (event) {
        ...
        var findRelViewExtPropertyVal = function (relviewCode, propertyName) {
          var propertyVal = '';
          $.each(app.fixrelview, function (index, obj) {
        ...
        $('.ok', relviewModal).click(function () {
          ...
          for (var i = 0; i < $('#relview-sel-table tr').length; i++) {
            relview = new Object();
            relview.typeName = $('input[name=relviewTname]', $("#relview-sel-table").find("tr").eq(i)).val();
            relview.typeCode = $('select[name=relviewTcode]', $("#relview-sel-table").find("tr").eq(i)).val();
            relview.name = $('input[name=relviewName]', $("#relview-sel-table").find("tr").eq(i)).val();
            relview.code = $('select[name=relviewCode]', $("#relview-sel-table").find("tr").eq(i)).val();
            relview.dispOrder = $('input[name=dispOrder]', $("#relview-sel-table").find("tr").eq(i)).val();
            if (relview.typeCode == '0') {
              $.each(app.fixrelview, function (j, obj) {
                if (obj['code'] == relview.code) {
                  relview.addPath = obj['addPath'];
                  relview.updatePath = obj['updatePath'];
                  relview.viewPath = obj['viewPath'];
                  relview.forms = obj['forms'];
                  return;
                }
              });
            }
            relviewArr.push(relview);
            relviewName += relview.name + ';';
          }
    ...
    var plugHandle = function () {
      ...
      var initPlugSelLoader = function (object) {
        $("select.triggerStage", object).each(function () {
          $(this).empty().append('<option value="">请选择触发阶段...</option>').selectloader({
            triggerStage: stepType == 'USERTASK' ? app.taskTriggerStage : app.exectutriggerStage
          });
        });
        $("select.selectloader-bpmlisteners", object).each(function () {
          $(this).empty().append('<option value="">请选择事件...</option>').selectloader({
            bpmlisteners: app.bpmlisteners
          });
        });
      };
  ...
  var init = function () {
    ...
    $('.commitway').radioloader({commitway: app.commitway});
    $('.assignway').radioloader({assignway: app.assignway});
    ...
    $(".candidateUsers").jstreemultiselect({
      candidateUsers: app.candidateUsers
    });
    $(".selectloader-autoassignstrategy").selectloader({
   		autoassignstrategy: app.autoassignstrategy
    });

    $('.portlet-title.clearfix', $('.searchPortlet')).css('display', 'none');
  };
  init();
...
$(function () {
   ...
   * 初始化数据
   */
  $('#mainPage,#tab1,#tab2').pagination({
    "first-store": app.firstStore
  });
  $('.filter-query-form', $('.searchPortlet')).css('display', 'block');
});