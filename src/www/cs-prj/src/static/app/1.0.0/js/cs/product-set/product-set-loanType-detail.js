        ...
        if(app.isOK(data)) {
          ...
          if((data.status == 'JH') && (data.hasXB != 'XB')) {
            if($('.cancleCret').attr('disabled') == false) {
              ...
              $('input,select,button,textarea').not('.creatNewVer,.dropdown-toggle,#loanTypeVersionList,#loTyNoHidden,#loKiCdHidden,#loTyNaHidden').attr('disabled', true);
              $('select').selectpicker('refresh');
              $('.add-sign,.delete-sign').addClass('hide');
            }
          } else if(data.status == 'XB') {
            $('input,select,button,textarea').not("#loanTypeVersionList,.dropdown-toggle,#releaseBtn,#loTyNo,.cancleCret").attr('disabled', true);
            $('select').selectpicker('refresh');
            $('.add-sign,.delete-sign').addClass('hide');
          } else {
            $('input,select,button,textarea').not("#loanTypeVersionList,.dropdown-toggle").attr('disabled', true);
            $('select').selectpicker('refresh');
            $('.add-sign,.delete-sign').addClass('hide');
          };
