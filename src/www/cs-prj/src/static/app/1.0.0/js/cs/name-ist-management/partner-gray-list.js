+function ($,app) {
    ...
    app.context.formInit = function (form) {
        /*   初始化弹窗里的下拉框   */
        form.find('[name="corpType"]').selectloader({'clientTpye': app.clientTpye});
        form.find('[name="corpPaperType"]').selectloader({'idType': app.idType});
        form.find('[name="pkId.blKeCd"]').selectloader({'listCode': app.listCode});
        form.find('[name="fromCd"]').selectloader({'listSource': app.listSource});
        form.find('[name="keStatus"]').selectloader({'state': app.state});
        form.find('[name="blCauCd"]').selectloader({'blCauCdList': app.blCauCdList});
        /*   初始化时间控件   */
    ...
    app.context.formInit1 = function (form) {
        ...
        form.find('#blKeCd').change(function(){
            if(($('#blKeCd option:selected').val())=="SJ"){
                ...
                $('#blVaCa').addClass('agencyCode').removeClass('isMobilePhone isIDCard businessLicense');
            }else {
                $('#blVaCa').removeClass('isMobilePhone isIDCard agencyCode businessLicense');
            }
        });
    ...
    $('#update').getModal({
        ...
        showBefore: function (modal) {
            app.context.showBefore({
                url: app.PARTNER_GRAY_GET_DATA_BY_ID,
                modal: modal,
        ...
        showAfter:function(modal){
            ...
            //根据名单代码判断名单数值的校验类
            var blKeCdSel =modal.find('#blKeCd option:selected').val();
            var blKeCdObj =modal.find('#blVaCa');
            if(blKeCdSel == "SJ") {
    ...
    $('#join').getModal({
        ...
        app.context.submit({
            modal: modal,
            url: app.PARTNER_GRAY_JOIN+'?blKeCd='+$('[type=radio]:checked').data('id')+'&'+'blVaCa='+$('[type=radio]:checked').data('shu'),
            text: '转入成功',
            isEasyModal: true
        }, app);
    });
    ...
    $('#remove').getModal({
        ...
        app.context.submit({
            modal: modal,
            url: app.PARTNER_GRAY_REMOVE+'?blKeCd='+$('[type=radio]:checked').data('id')+'&'+'blVaCa='+$('[type=radio]:checked').data('shu'),
            text: '移出成功',
            isEasyModal: true
        }, app);
    });
