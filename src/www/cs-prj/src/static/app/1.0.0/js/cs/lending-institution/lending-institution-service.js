+function ($,app) {
    ...
    var a=0;  //标志是修改还是新增
//  nc是可以修改的radio的初始化status;
//  yc是新增一行而且已经全部编辑
//  uped是选择修改后修改了
//  nw是新增一行的radio的初始化status
    ...
    */
    $('body').on('change', 'select.brNaList,.mlAcNo,.payAcNo,.co', function () {
        ...
        //判断是否是操作新增的一行
        var checkRadio=$(this).parents("tr").find('[type=radio]').data('status');
        if(checkRadio == 'nw' || checkRadio == 'yc'){
          ...
          $(this).parents("tr").find('[type=radio]').data('status','uped');
        }
    ...
    $('#save').getModal({
        size: 'modal-sm',
        statusArray:["yc","uped"],
        textArray:["确定要保存这条记录吗？","确定要保存这条记录吗？"],
        noHandleArray:["nc","nw"],
        ...
        selector: function () {
            ...
            var checkRadio = $('[type=radio]:checked').data('status');
            checkedRow.find(".mlAcNo,.payAcNo").addClass("required isBankCard notSpace");
            //判断选中的是新增的还是待修改的
            checkRadio == 'uped'?a=2:a=1;
