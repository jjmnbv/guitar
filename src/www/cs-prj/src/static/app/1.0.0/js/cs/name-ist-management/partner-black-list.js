+ function($, app) {
  ...
  $('#update').getModal({
    ...
    showBefore: function(modal) {
      app.context.showBefore({
        url: app.PARTNER_BLACK_GET_DATA_BY_ID,
        modal: modal,
        param: '?blKeCd=' + $('[type=radio]:checked').data('id') + '&' + 'blVaCa=' + $('[type=radio]:checked').data('shu'),
        /*callback:function(data){
            console.log("日期："+data.createDate);
            console.log(modal[0]);
            console.log($(modal[0]).find('input[name="createDate"]'));
            var inputText= $(modal[0]).find('input[name="createDate"]');
            console.log(inputText[0]);
           /!* $(inputText[0]).val(data.createDate);*!/
            console.log("232:"+$(inputText[0]).val());
        }*/
  ...
  $('#join').getModal({
    ...
    app.context.submit({
      modal: modal,
      url: app.PARTNER_BLACK_JOIN + '?blKeCd=' + $('[type=radio]:checked').data('id') + '&' + 'blVaCa=' + $('[type=radio]:checked').data('shu'),
      text: '转入成功',
      isEasyModal: true
    }, app);
  });
  ...
  $('#remove').getModal({
    ...
    app.context.submit({
      modal: modal,
      url: app.PARTNER_BLACK_REMOVE + '?blKeCd=' + $('[type=radio]:checked').data('id') + '&' + 'blVaCa=' + $('[type=radio]:checked').data('shu'),
      text: '移出成功',
      isEasyModal: true
    }, app);
  });
