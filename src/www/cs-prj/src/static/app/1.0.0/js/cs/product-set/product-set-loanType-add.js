$(function() {
  + function($, app) {
    ...
    var loanTypePlOpList = app.store.loanTypePlOpList;
    if(!!loanTypePlOpList) {
    ...
    //渠道
    /*新增一行*/
    var channelTable = $("#addChannel").tableWrapCurd({
    ...
    $('#removeChannel').getModal({
      ...
      selector: function() {
        return !!$('#removeChannel').parents('.portlet').find('[type=radio]:checked').length;
      },
      ...
      channelTable.delete(index, function(item, i) {
        var index = i + 1;
    ...
    $('#removeFee').getModal({
      ...
      footer: '<button data-dismiss="modal" type="button" class="btn blue" data-modal-ok>确定</button>',
      selector: function() {
        return !!$('#removeFee').parents('.portlet').find('[type=radio]:checked').length;
      },
     /* notSelectorLast: function(){
        var Trlength = $('#feeTableBody').find("tr").length;
        if(Trlength===1){
    ...
    $('#removeRepayMode').getModal({
      ...
      footer: '<button data-dismiss="modal" type="button" class="btn blue" data-modal-ok>确定</button>',
      selector: function() {
        return !!$('#removeRepayMode').parents('.portlet').find('[type=radio]:checked').length;
      },
      notSelectorLast: function(){
    ...
    //暂存功能
    $(document).on('click', '#templateForm', function(event) {
      $(this).attr("disabled", true);
      app.$post(app.TEMPLATE_SAVE_PRODUCT, $('#add-proSet-form').serialize()).done(function(data) {
        if(app.isOK(data)) {
          app.alertOK('暂存成功！');
          //暂存按钮可点击
          ...
          app.alertError(errors || failureText || '暂存失败！');
          /*lyf 暂存失败后，按钮可点击*/
          $("#templateForm").attr("disabled", false);
        }
      });
  ...
  var addFinance = function() {
    ...
    $('#saveForm', addForm).click(function(event) {
      if(validator.form()) {
        ...
        submit(app.COMMIT_PRODUCT_SET_ADD_DATA, $('#add-proSet-form').serialize(), event.target);
      } else {
        gotoError();
      }
      return false;
