+ function ($,app) {"use strict ";
   ...
   * 修改作品
   */
  var eidtWorks = function () {
    var editModal = $('#works-edit-modal');
    ...
    editModal.on('show.bs.modal', function (event) {
      var worksId = $(event.relatedTarget).data('worksId');
      if (!worksId) return;

      App.startPageLoading({ animate: true });
      var jqxhr = app.$getJSON(app.WORKS_VIEW + worksId);
      if (!jqxhr) {
        return;
      }
  ...
  var deleteWorks = function () {
    $(".main-page").delegate('[data-delete-works-id]', 'click', function () {
      var worksId = $(this).data('deleteWorksId');
      bootbox.confirm({title: '提示', message: '确定删除作品',
        callback: function (result) {
          ...
          App.startPageLoading({ animate: true });
          var jqxhr = app.$post(app.WORKS_REMOVE + worksId, {}, 'json');
          if (!jqxhr) {
            App.stopPageLoading();
            return;
          }
