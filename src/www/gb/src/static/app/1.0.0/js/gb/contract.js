+ function ($,app) {"use strict ";
  ...
  var addContract = function () {
    var addModal = $('#contract-add-modal');
   ...
   * 修改联系方式
   */
  var eidtContract = function () {
    var editModal = $('#contract-edit-modal');
    ...
    editModal.on('show.bs.modal', function (event) {
      var contractId = $(event.relatedTarget).data('contractId');
      if (!contractId) return;

      App.startPageLoading({ animate: true });
      var jqxhr = app.$getJSON(app.CONTRACT_VIEW + contractId);
      if (!jqxhr) {
        return;
      }
  ...
  var deleteContract = function () {
    $(".main-page").delegate('[data-delete-contract-id]', 'click', function () {
      var contractId = $(this).data('deleteContractId');
      bootbox.confirm({title: '提示', message: '确定删除联系方式',
        callback: function (result) {
          ...
          App.startPageLoading({ animate: true });
          var jqxhr = app.$post(app.CONTRACT_REMOVE + contractId, {}, 'json');
          if (!jqxhr) {
            App.stopPageLoading();
            return;
          }
