+ function ($,app) {"use strict ";
   ...
   * 修改工作
   */
  var eidtJob = function () {
    var editModal = $('#job-edit-modal');
    ...
    editModal.on('show.bs.modal', function (event) {
      var jobId = $(event.relatedTarget).data('jobId');
      if (!jobId) return;

      App.startPageLoading({ animate: true });
      var jqxhr = app.$getJSON(app.JOB_VIEW + jobId);
      if (!jqxhr) {
        return;
      }
  ...
  var deleteJob = function () {
    $(".main-page").delegate('[data-delete-job-id]', 'click', function () {
      var jobId = $(this).data('deleteJobId');
      bootbox.confirm({title: '提示', message: '确定删除工作',
        callback: function (result) {
          ...
          App.startPageLoading({ animate: true });
          var jqxhr = app.$post(app.JOB_REMOVE + jobId, {}, 'json');
          if (!jqxhr) {
            App.stopPageLoading();
            return;
          }
