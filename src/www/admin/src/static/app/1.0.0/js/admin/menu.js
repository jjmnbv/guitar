+ function ($, app) {
  ...
  var render = function (elem, options) {
    ...
    if (store) {
              ...
              action: function (data) {
                ...
                if (node.children && node.children.length > 0) {
                  ...
                  bootbox.confirm({
                    ...
                    callback: function (e) {
                      if (e) {
                        var jqxhr = app.$post(app.MENU_REMOVE + node.id, {}, 'json');
      ...
      $tree.on('changed.jstree', function (e, jstreeData) {
        if (jstreeData.action == 'select_node') {
          ...
          if (node.id == '-1') {
            ...
            var jqxhr = app.$get(app.MENU_VIEW + node.id + '.json', 'json');
            if (!jqxhr) {
              return false;
            }
            jqxhr.done(function (data) {
              ...
              $("#add_action_btn a").unbind('click').click(function () {
                ...
                handleMenuAction($tr, app.MENU_ACTION_CREATE);
              });
      ...
      var refeshActionList = function (id) {
        var jqxhr = app.$get(app.MENU_VIEW + id + '.json', 'json');
        if (!jqxhr) {
          return false;
        }
        jqxhr.done(function (data) {
        ...
        $('td a.edit-action', action_tbody).unbind('click').click(function () {
          ...
          $("td", tr).each(function () {
            if ($(this).data("name") != 'operations' && $(this).data("name") != 'status') {
              ...
              handleMenuAction(tr, app.MENU_ACTION_UPDATE);
        ...
        $('td a.del-action', action_tbody).unbind('click').click(function () {
          ...
          bootbox.confirm({
            title: '提示', message: '确定删除页面资源', callback: function (e) {
              if (e) {
                var jqxhr = app.$post(app.MENU_ACTION_REMOVE + tr.data("actionId") + '.json', {}, 'json');
                if (!jqxhr) return false;
                jqxhr.done(function (data) {
                  if (app.isOK(data)) {
      ...
      var handleMenuAction = function (tr, uri) {
        ...
        $('td a.save', tr).unbind('click').click(function () {
          ...
          var jqxhr = app.$post(uri, data, 'json');
          if (!jqxhr) {
            return false;
          }
          jqxhr.done(function (data) {
  ...
  var init = function () {
    $.getJSON(app.MENU_JSTREE, function (json) {
