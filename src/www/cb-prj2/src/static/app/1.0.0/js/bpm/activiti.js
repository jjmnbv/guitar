+ function($, app){
  ...
  var enableFlowHandle = function() {
    ...
    $(".main-page").delegate('.activiti-enable', 'click', function(){
      ...
      App.startPageLoading({ animate: true });
      var jqxhr = app.$post(app.base+'/workflow/flows/enable/' + id, {}, 'json');
      if (!jqxhr) return false;
      jqxhr.done(function (data) {
        App.stopPageLoading();
        if (app.isOK(data)) {
  ...
  var disableFlowHandle = function() {
    $(".main-page").delegate('.activiti-disable', 'click', function(){
      ...
      App.startPageLoading({ animate: true });
      var jqxhr = app.$post(app.base+'/workflow/flows/disable/' + id, {}, 'json');
      if (!jqxhr) return false;
      jqxhr.done(function (data) {
        App.stopPageLoading();
        if (app.isOK(data)) {
  ...
  var removeFlowInfo = function() {
    ...
    $(".main-page").delegate('.remove', 'click', function(){
      ...
      bootbox.confirm({title: '提示', message: tmsg, callback: function (e) {
        if (e) {
          App.startPageLoading({ animate: true });
          var jqxhr = app.$post(app.base+'/workflow/flows/remove/' + id, {}, 'json');
          if (!jqxhr) return false;
          jqxhr.done(function (data) {
            App.stopPageLoading();
            if (app.isOK(data)) {
    ...
    $('.activiti-create').on('click',function(){
      ...
      window.open(path+'/workflow/flows/create','_blank');
    });
  ...
  $(document).on('click','.activiti-copy',function(){
    ...
    if(id){
      $.ajax({
          url:base +"/workflow/flows/copy/"+id.val(),
          type:"POST",
          timeout:10000,
          async: true,
          dataType:"json",
  ...
  var stepConfig = function() {
    ...
    stepListModal.on('show.bs.modal', function (event) {
      ...
      var jqxhr = app.$get(app.base + '/workflow/steps/' + flowInfoId, 'json');
      if (!jqxhr) {
        return;
      }
