...
//    return this.optional(element) || /[^~!@#$%^&*()_+=`{}\[\]\-|\\:;'<>,.\/? ]/.test(value);
  ...
  $.validator.addMethod('telNumber', function(value, element) {
    return this.optional(element) || (/^1[3|5|7|8|][0-9]{9}$/.test(value) || /^((\+86)?|\(\+86\)|\+86\s)0?([1-9]\d-?\d{6,8}|[3-9][13579]\d-?\d{6,7}|[3-9][24680]\d{2}-?\d{6})(-\d{3})?$/.test(value));
  }, '请输入正确的电话号码！');
  ...
  //需要大于与其关联的值
  validator.addMethod("gtRelate",function(value,element){
	  $('input[data-gtRelate]').data('num',app.unformatMoney($('input[data-gtRelate]').val()));
	  var gtRelateVal = app.unformatMoney($('input[data-gtRelate]').val())
	  console.log(gtRelateVal);
	  var valueNum = app.unformatMoney(value);
	  console.log(value);
	  var compareRe = (valueNum > gtRelateVal);
	  console.log(compareRe);
    return this.optional(element) || compareRe;
  },function(value,element){
	  var compareLabel = '需大于'+$('input[data-gtRelate]').parent().prev('label').html();
    return compareLabel;
  });
 //需要小于与其关联的值
  validator.addMethod("ltRelate",function(value,element){
	  $('input[data-ltRelate]').data('num',app.unformatMoney($('input[data-ltRelate]').val()));
	  var ltRelateVal = $('input[data-ltRelate]').data('num');
	  var valueNum = app.unformatMoney(value);
	  var compareRe = (valueNum <= ltRelateVal);
    return this.optional(element) || compareRe;
  },function(value,element){
	  var compareLabel = '需小于'+$('input[data-ltRelate]').parent().prev('label').html();
    return compareLabel;
  });
