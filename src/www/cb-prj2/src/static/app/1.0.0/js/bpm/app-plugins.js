...
+function ($, app) {
    ...
    var
        ...
        render = function (elem, pageNumber) {
            ...
            url = url + (pageQueryString ? '?' + pageQueryString + '&thiznow=' + (new Date().getTime()) : '');
                ...
                if (json.resultCode) {
                    if (!app.isOK(json)) {
                        ...
                        if(json.content.length<=0){
                            ...
                            //zj 补充有数据时的情况
                            $("#mainPage").find(".pagination-container").removeClass("noDataTbody");
                        }
                    }
                }
...
//                      var queryAllParamString= $queryAll.prev().find('input').val();
          ...
          /*  $queryClose.click(function () {
            ...
            $container.delegate('.goPage', 'click', function () {
                render($container, $container.find('.pageVal').val());
                return false;
            });
...
+function ($, app) {
    ...
    var render = function (elem, options, callBack) {
        //
        //获取数据源
        //
    ...
    $.fn.jstreemultiselect = function (options, callBack) {
        return this.each(function () {
            render($(this), options, callBack);
        });
    };
