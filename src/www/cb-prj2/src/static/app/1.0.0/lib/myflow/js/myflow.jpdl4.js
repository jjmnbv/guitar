(function ($, app) {
          ...
          signalMangModal.on('show.bs.modal', function (event) {
            ...
            var removeTr = function (tbody, trIndex) {
              ...
              if ($('tr', tbody).length == 0) {
                $(tbody).html('<tr class="no-data"><td colspan="5" align="center"></td></tr>');
              }
            };
          });
  ...
  $.extend(true, myflow.config.tools.states, {
    ...
    eventbased_gateway: {
      ...
      text: { text: '事件网关' },
      img: { src: '/static/app/1.0.0/lib/myflow/img/32/cancel.png', width: 48, height: 48 },
      attr: { width: 50, heigth: 50 },
      props: {
        text: {
      ...
      props: {
        ...
        nodeForm: {
          name: 'nodeForm', label: '节点表单', value: '', editor: function () {
            // return new myflow.editors.multiSelectEditor(
            // 	[{name:'请假审批',value:'leave-audit.form'}
            // 	,{name:'节点表单',value:'task-form.form'}]);
            return new myflow.editors.multiSelectEditor(app.fixrelview);
          }
        },
        editAuth: {
        ...
        todoInform: {
          name: 'todoInform', label: '待办通知(radio)', value: '', editor: function () {
            return new myflow.editors.selectEditor(dict.choice2);
          }
        },
        isAllowTakeBack: {
          name: 'isAllowTakeBack', label: '允许拿回(radio)', value: '', editor: function () {
            return new myflow.editors.selectEditor(dict.choice2);
          }
        },
        isAllowTransfer: {
          name: 'isAllowTransfer', label: '允许转办(radio)', value: '', editor: function () {
            return new myflow.editors.selectEditor(dict.choice2);
          }
        },
        isAllowReturn: {
          name: 'isAllowReturn', label: '允许退回(radio)', value: '', editor: function () {
            return new myflow.editors.selectEditor(dict.choice2);
          }
        },
        isAllowCancel: {
        ...
        cancelTime: {
          name: 'cancelTime', label: '挂起时长', value: '', editor: function () {
            return new myflow.editors.inputEditor();
          }
        },
        isAllowRefuse: {
        ...
        isAllowBackTo: {
          name: 'isAllowBackTo', label: '允许打回(radio)', value: '', editor: function () {
            return new myflow.editors.selectEditor(dict.choice2);
          }
        },
        isAllowEndorse: {
          name: 'isAllowEndorse', label: '允许加签(radio)', value: '', editor: function () {
            return new myflow.editors.selectEditor(dict.choice2);
          }
        },
        isAllowUrge: {
          name: 'isAllowUrge', label: '手工催办(radio)', value: '', editor: function () {
            return new myflow.editors.selectEditor(dict.choice2);
          }
        },
        nodeManageDateLimit: {
        ...
        allowExpireDealing: {
          name: 'allowExpireDealing', label: '启动过期处理(radio)', value: '', editor: function () {
            return new myflow.editors.selectEditor(
              [{ name: '请选择', value: '' }
                , { name: '一直催办', value: 1 }
                , { name: '不处理', value: 2 }
        ...
        callSubFlow: {
          name: 'callSubFlow', label: '调用子流程（暂不做）(radio)', value: '', editor: function () {
            return new myflow.editors.selectEditor(dict.choice2);
          }
        },
        subFlowSign: {
          name: 'subFlowSign', label: '子流程标记（暂不做）(radio)', value: '', editor: function () {
            return new myflow.editors.selectEditor(dict.choice2);
          }
        },
        ccObject: {
          name: 'ccObject', label: '抄送对象(此为多选,控件之后要优化)(multiple select)', value: '', editor: function () {
        ...
        isRunScript: {
          name: 'isRunScript', label: '是否运行脚本(radio)', value: '', editor: function () {
            return new myflow.editors.selectEditor(dict.choice2);
          }
        },
        runScript: {
  ...
  $.extend(true, myflow.config.path.props, {
    ...
    isRunScript2: {
      name: 'isRunScript2', label: '是否运行脚本(radio)', value: '', editor: function () {
        return new myflow.editors.selectEditor(dict.choice);
      }
    },
    runScript2: {
