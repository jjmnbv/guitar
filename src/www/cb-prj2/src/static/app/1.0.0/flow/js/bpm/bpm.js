+(function($){
  $.Bpm = {
    ...
    startProcess: function(flowKey, fn) {
      $this = this;
      var taskId = '';
      $this.ajaxPost('/workflow/startext/' + flowKey, 'get', {}, false, function(data) {
        taskId = data.taskId;
    ...
    startProcessExt: function(flowKey, formIdArr, fn) {
      $this = this;
      var taskId = '';
      $this.ajaxPost('/workflow/startext', 'post', JSON.stringify({formIds: formIdArr, flowKey: flowKey}), false, function(data) {
        taskId = data.taskId;
        $this.options.taskId = data.taskId;
        if (fn) {
          fn(data);
    ...
    findTaskStatus: function(fn) {
      ...
      var taskStatus = '';
      this.ajaxPost('/workflow/taskstatus/' + taskId, 'get', {}, false, function(data) {
        if (app.isOK(data)) {
    ...
    handle: function(fn) {
      var taskId = this.options.taskId;
      this.ajaxPost('/workflow/handleexter/' + taskId, 'get', {}, false, function(data) {
    ...
    viewTask: function(fn) {
      var taskId = this.options.taskId;
      this.ajaxPost('/workflow/viewtask/' + taskId, 'get', {}, false, function(data) {
        if (app.isOK(data) && data.relView != null) {
          if (fn) {
            fn(data);
            return;
    ...
    initFormExt: function(opinionFormId, callback, opinionInitCall) {
      $this = this;
      var jqxhr = app.$get(app.base + '/workflow/handledata/' + this.options.taskId, 'json');
      if (!jqxhr) {
        return false;
      }
      jqxhr.done(function (data) {
        ...
        if (data.opinion) {
          if (opinionInitCall) {
            opinionInitCall(data.opinion);
          } else {
            $('#' + opinionFormId).deserializeObject(data.opinion);
          }
        }

        $.each(relView.forms, function (index, item) {
          var form = $('#'+item.formId);
          if (form.length <= 0 || !item.viewUrl) {
            return;
          }
    ...
    getMileStones: function (fn) {
      $this = this;
      $this.ajaxPost(
        app.base + '/workflow/milestone/' + $this.options.taskId,
        'get',
        {},
        true,
    ...
    getHistoryTasks: function (fn) {
      $this = this;
      $this.ajaxPost(
        app.base + '/workflow/histasks/' + $this.options.taskId,
        'get',
        {},
        true,
        function (data, textStatus, jqXHR) {
    ...
    saveOpinion: function(opinionContent) {
      ...
      var postFlag = false;
      $this.ajaxPost(
        app.base + '/workflow/opinion/save',
        'post',
    ...
    submitHandleExt: function(formIdArr, opinionFormId, opinionContent, fn) {
      $this = this;
      var nextAssignee, formIdArr = [];
      $this.options.message = $('select[name="opinionId"]').val();
      var flag = true;

      $this.ajaxPost(app.base + '/workflow/manualcommityn',
        'post',
        JSON.stringify({ stepKey: $this.options.stepKey, taskId: $this.options.taskId }),
        false,
        function (data, textStatus, jqXHR) {
          if (data) {
            $this.ajaxPost(app.base + '/workflow/nextstep/candidates',
              'post',
            ...
            flag = $this.commitExt(formIdArr, opinionFormId, opinionContent, false, fn);
          }
      });
    },
    refuseHandle: function(formIdArr, opinionFormId, opinionContent, fn) {
      ...
      $this.ajaxPost(app.base + '/workflow/refuse',
        'post',
    ...
    commitExt: function(formIdArr, opinionFormId, opinionContent, isAsync, fn) {
      $this = this;
      var flag = true;
      ...
      $.ajax({
        ...
        data: JSON.stringify({ flowKey: $this.options.flowKey, stepKey: $this.options.stepKey, taskId: $this.options.taskId, formIds: formIdArr, message: $this.options.message, comment: comment, nextAssignee: '', opinionFormId: opinionFormId, opinion: opinionContent}),
        success: function (data, textStatus, jqXHR) {
          if (fn) {
            fn(data);
            return;
    ...
    initCandidatesSelModalExt: function(processUrl) {
      ...
      candidatesSelectModal.on('show.bs.modal', function (event) {
        $("button.ok", candidatesSelectModal).attr("disabled", true);
        $this.ajaxPost(
          app.base + '/workflow/'+ $this.options.flowKey +'/handlers',
          'get',
          JSON.stringify({ }),
          true,
          function (data, textStatus, jqXHR) {
    ...
    beforeissue: function() {
      $this =this;
      $this.initCandidatesSelModalExt('/workflow/beforeIssue/' + $this.options.taskId);
      $this.showCandidatesSelModalExt();
    },
    /**后加签 */
    afterissue: function() {
      $this =this;
      $this.initCandidatesSelModalExt('/workflow/afterIssue/' + $this.options.taskId);
      $this.showCandidatesSelModalExt();
    },
    /**转办 */
    transfertask: function() {
      $this =this;
      $this.initCandidatesSelModalExt('/workflow/transferTask/' + $this.options.taskId);
      $this.showCandidatesSelModalExt();
    },
    /**填充流程审批意见**/
    ...
    rollback: function() {
      $this = this;
      var rollbackFormHtml = '<div class="modal fade" id="rollback_form" tabindex="-1" role="dialog" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">退回意见</h4></div><div class="modal-body"><form action="${base}/workflow/rollback" method="POST"><div class="row"><div class="col-md-12"><div class="form-group"><div class="input-group"><span class="input-group-addon">意见</span><textarea name="comment" class="form-control" rows="5"></textarea></div></div></div></div></form></div><div class="modal-footer"><button type="button" class="btn btn-success ok">提交</button><button type="button" class="btn btn-default" data-dismiss="modal">取消</button></div></div></div></div>';
      ...
      $('#rollback_form .ok').on('click', function () {
        ...
        $.ajax({
          ...
          dataType: 'json',
          cache: false,
          url: app.base + '/workflow/rollback/' + $this.options.taskId,
    ...
    getRollbackList: function() {
      ...
      var rollbackList = [];
      $this.ajaxPost(
        app.base + '/workflow/rollbacklist/' + $this.options.taskId,
        'get',
        {},
        false,
        function (data, textStatus, jqXHR) {
    ...
    rollbackToWithTarget: function(comment, taskId,fn) {
      ...
      $.ajax({
        ...
        dataType: 'json',
        cache: false,
        url: app.base + '/workflow/rollbackwithcomment/' + taskId,
    ...
    initRollbackToModal: function() {
      $this = this;
      var rollbackFormHtml = '<div class="modal fade" id="rollbackto_form" tabindex="-1" role="dialog" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button><h4 class="modal-title">退回意见</h4></div><div class="modal-body"><form action="/workflow/rollback" method="POST"><div class="row"><div class="col-md-12"><div class="form-group"><div class="input-group"><span class="input-group-addon">退回节点</span><select id="rollbackTaskId" name="rollbackTaskId" class="form-control wordbooks" data-selectloader-val="" required> <option value="">请选择...</option> </select> </div></div><div class="form-group"> <div class="input-group"><span class="input-group-addon">退回意见</span><input type="hidden" name="taskId" value="${taskId}"><textarea name="comment" class="form-control" rows="5"></textarea> </div></div></div></div></form></div><div class="modal-footer"><button type="button" class="btn btn-success ok">提交</button><button type="button" class="btn btn-default" data-dismiss="modal">取消</button></div></div></div></div>';
    ...
    retract:function() {
      $this = this;
      $this.ajaxPost(
        app.base + '/workflow/retract/' + $this.options.taskId,
        'post',
        {},
        true,
        function (data, textStatus, jqXHR) {
