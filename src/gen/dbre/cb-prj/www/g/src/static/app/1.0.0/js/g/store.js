+ function ($,app) {"use strict ";
   ...
   * 修改门店
   */
  var eidtStore = function () {
    var editModal = $('#store-edit-modal');
    ...
    editModal.on('show.bs.modal', function (event) {
      var storeId = $(event.relatedTarget).data('storeId');
      if (!storeId) return;

      App.startPageLoading({ animate: true });
      var jqxhr = app.$getJSON(app.STORE_VIEW + storeId);
      if (!jqxhr) {
        return;
      }
  ...
  var deleteStore = function () {
    $(".main-page").delegate('[data-delete-store-id]', 'click', function () {
      var storeId = $(this).data('deleteStoreId');
      bootbox.confirm({title: '提示', message: '确定删除门店',
        callback: function (result) {
          ...
          App.startPageLoading({ animate: true });
          var jqxhr = app.$post(app.STORE_REMOVE + storeId, {}, 'json');
          if (!jqxhr) {
            App.stopPageLoading();
            return;
          }
