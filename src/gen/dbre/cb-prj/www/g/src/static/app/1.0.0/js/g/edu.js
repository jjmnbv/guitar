+ function ($,app) {"use strict ";
   ...
   * 修改
   */
  var eidtEdu = function () {
    var editModal = $('#edu-edit-modal');
    ...
    editModal.on('show.bs.modal', function (event) {
      var eduId = $(event.relatedTarget).data('eduId');
      if (!eduId) return;

      App.startPageLoading({ animate: true });
      var jqxhr = app.$getJSON(app.EDU_VIEW + eduId);
      if (!jqxhr) {
        return;
      }
   ...
   * 删除
   */
  var deleteEdu = function () {
    $(".main-page").delegate('[data-delete-edu-id]', 'click', function () {
      var eduId = $(this).data('deleteEduId');
      bootbox.confirm({title: '提示', message: '确定删除',
        callback: function (result) {
          ...
          App.startPageLoading({ animate: true });
          var jqxhr = app.$post(app.EDU_REMOVE + eduId, {}, 'json');
          if (!jqxhr) {
            App.stopPageLoading();
            return;
          }
