+ function($, app){
	...
	var postFormIds = function(formIdArr) {
		...
		$.ajax({
			...
			dataType : 'json',
			cache: false,
			url: app.base+'/flowengine/formids/update',
	...
	var submitHandle = function() {
	    ...
	    if(!flag) {
			...
			ajaxPost(app.base+'/flowengine/manualcommited',
				'post',
				JSON.stringify({stepKey: stepKey, taskId: taskId}),
				false,
				function(data, textStatus, jqXHR) {
					if (data) {
						ajaxPost(app.base+'/flowengine/nextstep/candidates',
							'post',
        ...
        $.ajax({
  			...
  			dataType : 'json',
  			cache: false,
  			url: app.base+'/flowengine/complete',
	...
	var rollback = function(){
		...
		var taskId2 = rollbackForm.find('input[name="taskId"]').val();
		
		ajaxPost(app.base+'/activiti/rollbacklist/'+taskId2,
				'get',
				{},
				true,
				function(data, textStatus, jqXHR) {
					...
					rollbackForm.find('#rollbackTaskId').html(html);
				}
		);
		...
		$('#rollback_form .ok').on('click',function(){
			var taskId = $.trim(rollbackForm.find('#rollbackTaskId').val());
			if(!taskId) {
			...
			$.ajax({
	  			...
	  			dataType : 'json',
	  			cache: false,
	  			url: app.base+'/activiti/rollbackwithcomment/'+taskId,
	  			data: {comment:comment,currTaskId:taskId2},
	  			success: function(data, textStatus, jqXHR) {
	  				if(app.isOK(data)) {
	  					App.stopPageLoading();
	  					app.alertOK('提交成功.');
	...
	var beforeissue = function(){
		...
		var beforeissue = $('#beforeissue_form');
		$('.beforeissue').click(function() {
			beforeissue.modal('show');
	    });
		
		
		$('#beforeissue_form .ok').on('click',function(){
			
			var assignee = '';
			$.each($('.jstree-clicked'),function(index,item){
				assignee += $(item).parent().attr('id')+',';
			});
			if(assignee){
				assignee = assignee.substring(0,assignee.length -1);
			}else{
				app.alertError("请选择人员!");
		    	return;
			}
			
			var taskId = beforeissue.find('input[name="taskId"]').val();
			$.ajax({
	  			type : 'post',
	  			async : true,
	  			dataType : 'json',
	  			cache: false,
	  			url: app.base+'/activiti/beforeIssue/'+taskId,
	...
	var afterissue = function(){
		...
		$('#beforeissue_form .ok').on('click',function(){
			...
			$.ajax({
	  			...
	  			dataType : 'json',
	  			cache: false,
	  			url: app.base+'/activiti/afterIssue/'+taskId,
	  			data: {assignee:assignee},
	  			success: function(data, textStatus, jqXHR) {
	  				if(app.isOK(data)) {
	  					App.stopPageLoading();
	...
	var transfertask = function(){
		...
		$('#beforeissue_form .ok').on('click',function(){
			...
			$.ajax({
	  			...
	  			dataType : 'json',
	  			cache: false,
	  			url: app.base+'/activiti/transferTask/'+taskId,
	  			data: {assignee:assignee},
	  			success: function(data, textStatus, jqXHR) {
	  				if(app.isOK(data)) {
	  					App.stopPageLoading();
	...
	var cancelTask = function(){
		...
		$(document).on('click','#cancelTask',function(){
			$.ajax({
	  			...
	  			dataType : 'json',
	  			cache: false,
	  			url: app.base+'/activiti/cancel/'+taskId,
	  			success: function(data, textStatus, jqXHR) {
	  				if(app.isOK(data)) {
	  					App.stopPageLoading();
	  					app.alertOK('取消成功.');
