+ function($, app){
	...
	function fillMessage(name) {
		if($('select[name="'+name+'"]').length > 0){
			if(message){
				$('select[name="'+name+'"]').html('<option>'+message+'</option>');
			}
		}
	};
	...
	function fillComment(name){
		if($('textarea[name="'+name+'"]').length > 0){
			$('textarea[name="'+name+'"]').html(comment);
		}
	}
	...
	var rollback = function(){
		...
		$('#rollback_form .ok').on('click',function(){
			...
			$.ajax({
	  			...
	  			dataType : 'json',
	  			cache: false,
	  			url: app.base+'/activiti/rollback/'+taskId,
	...
	var activeTask = function(){
		...
		$(document).on('click','#activeTask',function(){
			$.ajax({
	  			...
	  			dataType : 'json',
	  			cache: false,
	  			url: app.base+'/activiti/active/'+taskId,
	  			success: function(data, textStatus, jqXHR) {
	  				if(app.isOK(data)) {
	  					App.stopPageLoading();
	  					app.alertOK('激活成功.');
	...
	var retract = function(){
		...
		$(document).on('click','.retract',function(){
			$.ajax({
	  			...
	  			dataType : 'json',
	  			cache: false,
	  			url: app.base+'/activiti/retract/'+taskId,
	  			success: function(data, textStatus, jqXHR) {
	  				if(app.isOK(data)) {
	  					App.stopPageLoading();
	  					app.alertOK('撤销成功.');
			...
			$.each($('#viewcodeContainer > div'), function(index, item) {
				...
				if(viewType == 'nonfix') {
					...
					jqxhr.done(function (data) {
						...
						options['callback'] = function() { loadNonfixFormData(id, $(form));fillMessage('opinionId');fillComment('opinionNote'); };
						$(form).jsform(options);
					});
				}
			});
