...
<c:set target="${self}" property="title">已办任务</c:set>
...
<c:set target="${self.js}" property="main">
  <script src="${self.path.app}/js/bpm/flowprocess-donelist.js"></script>
  <script src="${self.path.app}/js/bpm/bpm.js"></script>
  <script>
  	$(function () {
  		
		var mileStone = [
		  {'name':'mileStone','text':'是'},
		  {'name':'','text':'否'}
		];
		app.registerTextHelper('mileStone', mileStone, 'name', 'text');  	
...
<c:set target="${self.content}" property="paginationContainer">
  <div class="portlet-title">
    <div class="caption">
      <span><i class="fa fa-list"></i> 已办任务列表 </span>
    </div>
    <div class="actions">
  ...
  <div class="portlet-body pagination-container" data-page-first-store="first-store" data-page-url="${base}/workflow/donetask/page" data-page-template-id="flows-page-template"></div>
</c:set>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td> {{description}} </td>
						...
						<td>
				 			<a href="javascript:;" class="flow-done-view" data-deployid="{{deployId}}" data-id="{{taskId}}" data-flowKey="{{flowKey}}" data-stepKey="{{stepKey}}">查看</a> 
							<a href="javascript:;" class="flow-done-retract" data-id="{{taskId}}" data-flowKey="{{flowKey}}" data-stepKey="{{stepKey}}">撤销</a>
						</td>
          </tr>
          {{/each}}
        </tbody>
