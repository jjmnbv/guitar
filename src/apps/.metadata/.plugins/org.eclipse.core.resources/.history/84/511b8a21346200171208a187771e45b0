...
<c:set target="${self.js}" property="main">
  ...
  <script>
  	var base = '${base}';  
		...
		var taskTriggerStage = [{code:'create',name:'开始'}, {code:'assignment',name:'签收'}, {code:'complete',name:'结束'}, {code:'delete',name:'删除'}, {code:'suspend',name:'挂起'}, {code:'suspend_delete',name:'挂起结束流程'}, {code:'active',name:'激活'}, {code:'revoke',name:'撤销'}];
		var exectutriggerStage = [{code:'start',name:'开始'}, {code:'end',name:'结束'}];
  	$(function () {
      var deployStatus = [
        {'name':'0','text':'已保存'},
        {'name':'1','text':'已部署'},
      ...
      $(".selectloader-positions").selectloader({
      	positions: <spring:eval expression="@FN_BPM.positionsToJSON()" />
      });
      $(".candidateUsers").jstreemultiselect({
        candidateUsers: <spring:eval expression="@FN_BPM.organuserNameJsTreeJSON()" />
      });
      
      fixrelview = <spring:eval expression="@FN_BPM.fixFormToJSON()" />;
...
<c:set target="${self.content}" property="paginationContainer">
  ...
  <div class="portlet-body pagination-container" data-page-first-store="first-store" data-page-url="${base}/workflow/flows/page" data-page-template-id="activiti-page-template"></div>
</c:set>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="${base}/workflow/flows/updatex" method="POST" role="form" class="form-horizontal">
            <div class="form-body">
          ...
          <tr class="normal">
            <td width="30%">
              <input type="hidden" name="relviewTname" value="{{typeName}}"/>
							<select name="relviewTcode" class="form-control relviewtype" data-selectloader-vkey="code" data-selectloader-tkey="name" data-selectloader-store="relviewtype" data-text-input="input[name=relviewTname]" data-selectloader-val="{{typeCode}}" tabindex="1">
                <option value="">请选择视图类型...</option>
              </select>
            <td width="50%">
							<input type="hidden" name="relviewName" value="{{name}}"/>
							<select name="relviewCode" class="form-control relviewsel" data-selectloader-vkey="code" data-selectloader-tkey="name" data-selectloader-store="relview" data-text-input="input[name=relviewName]" data-selectloader-val="{{code}}" data-placeholder="视图..." tabindex="1">
